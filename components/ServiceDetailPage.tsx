...
import { ArrowLeft, Calendar, FileText, Paperclip, CheckCircle, AlertTriangle, Circle, Search, Activity, LayoutTemplate, ListTodo, Image as ImageIcon, Briefcase, Plus, Download } from 'lucide-react';
...
export const ServiceDetailPage: React.FC<ServiceDetailPageProps> = ({ order, allOrders = [], onBack, onCreateFollowUp, onOrderClick }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("TRANICO LTD", 14, 20);

    // Subtitle
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold"); // Using bold for the subtitle "GYM SERVICES & EQUIPMENT"
    doc.setTextColor(50, 50, 50);
    doc.text("GYM SERVICES & EQUIPMENT", 14, 26);
    // Draw a line under the logo section or just leave spacing
    
    // Address / Contact Info (Left Side)
    ...
    doc.text("Tranico LTD", 14, leftY);
    leftY += lineHeight;
    doc.text("91 Spoondell", 14, leftY);
    leftY += lineHeight;
    doc.text("LU63JF Dunstable", 14, leftY);
    leftY += lineHeight;
    doc.text("Telephone: 07541540207", 14, leftY);
    leftY += lineHeight;
    doc.text("Email: info@tranico.co.uk", 14, leftY);
    leftY += lineHeight;
    doc.text("Web: www.tranico.co.uk", 14, leftY);

    // -- Job Info (Right Side) --
    // We'll align this block to start around x=110
    const rightX = 110;
    let rightY = 35;
    
    // Helper for bold label + normal text
    const addInfoRow = (label: string, value: string) => {
      doc.setFont("helvetica", "bold");
      doc.text(label, rightX, rightY);
      doc.setFont("helvetica", "normal");
      // Align value a bit to the right of the label
      doc.text(value, rightX + 40, rightY); 
      rightY += 6; // slightly larger spacing for main info
    };

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(10);

    addInfoRow("Site Name:", order.customer.companyName);
    addInfoRow("Job number:", order.jobNumber);
    addInfoRow("Date:", order.date);
    addInfoRow("Engineer:", order.assignedEngineer?.name || "Unassigned");
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("SERVICE INSPECTION RECORD", 14, tableStartY - 5);

    // Prepare Data
    const tableBody = (order.serviceItems || []).map(item => {
      // Logic: If Fixed -> "OK". Else -> Engineer Comments or "Issue"
      let comment = "";
      if (item.status === 'Fixed') {
        comment = "OK";
      } else {
        comment = item.engineerComments || "Issue Reported";
      }

      return [
        item.serialNumber,
        item.name,
        // We assume "Location" corresponds to Site Name, but if user meant a column in table:
        // "contain the mchine name, serial number, location..."
        // Since machines are usually at the same site, we'll put Site Name in header. 
        // If specific machine location (e.g. "Room A") exists, we'd map it here.
        // We'll stick to the requested columns in the screenshot + user request context.
        comment
      ];
    });
    ...
    autoTable(doc, {
      startY: tableStartY,
      head: [['SERIAL NUMBER', 'MACHINE', 'COMMENTS']],
      body: tableBody,
      theme: 'plain', // To match the "Excel" look more closely or use 'grid'
      styles: {
        fontSize: 9,
        cellPadding: 3,
        lineColor: [0, 0, 0],
      ...
      columnStyles: {
        0: { cellWidth: 50 }, // Serial Number
        1: { cellWidth: 60 }, // Machine
        2: { cellWidth: 'auto' } // Comments
      }
    });

    // Save
    doc.save(`${order.jobNumber}_Service_Report.pdf`);
  };
